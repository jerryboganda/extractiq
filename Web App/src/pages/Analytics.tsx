...
import type { ConfidenceDistributionBucket } from "@/lib/api-types";
...
export default function Analytics() {
  ...
  const analyticsCostBreakdown = useMemo(() => costBreakdown ?? [], [costBreakdown]);
  const costKeys = useMemo(
    () => Array.from(new Set(analyticsCostBreakdown.flatMap((entry) => Object.keys(entry).filter((key) => key !== "week")))),
  ...
  const noAnalyticsData = analyticsTimeSeries.length === 0
    && analyticsConfidenceDistribution.length === 0
    && analyticsProviderComparison.length === 0
    && analyticsProcessingTimeTrend.length === 0
    && analyticsCostBreakdown.length === 0;
    ...
    <div className="space-y-6">
      ...
      {noAnalyticsData ? (
        <EmptyState
          ...
          description="Analytics will populate after documents are processed and reviewed."
        />
      ) : (
        <>
                    ...
                    <BarChart data={analyticsCostBreakdown} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                      ...
                      {costKeys.map((key, index) => (
                        <Bar
                          ...
                          dataKey={key}
                          stackId="cost"
                          fill={`hsl(var(${["--primary", "--info", "--warning", "--success"][index % 4]}))`}
                          name={key}
                          radius={index === costKeys.length - 1 ? [3, 3, 0, 0] : undefined}
                        />
                      ))}
                    </BarChart>
                  </ResponsiveContainer>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-8 sm:gap-12">
                <ResponsiveContainer width={220} height={220}>
                  <PieChart>
                    ...
                    >
                      {analyticsConfidenceDistribution.map((entry: ConfidenceDistributionBucket, index: number) => (
                ...
                <div className="space-y-3">
                  {analyticsConfidenceDistribution.map((bucket: ConfidenceDistributionBucket) => (
                    <div key={bucket.range} className="flex items-center gap-3">
                      <div className="h-3 w-3 rounded-full" style={{ background: bucket.fill }} />
                      <span className="text-sm text-muted-foreground w-16">{bucket.range}</span>
                      <span className="text-sm font-semibold">{bucket.count.toLocaleString()}</span>
                    </div>
                  ))}
                </div>
              </div>
